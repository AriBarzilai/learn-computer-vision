...
def get_index_matrix():
    ...
    X = np.concatenate([np.ones_like(x), 
                    x, 
                    y, 
                    x**2, 
                    x*y, 
...
def fix_raw_im(b, vig_im):
    ...
    # remove cases where value is 0 to prevent division by 0
    #epsilon = 1e-6
    #rec_calib_map = np.maximum(res_2d, epsilon)
    rec_calib_map_rgb = np.dstack([rec_calib_map]*3)
    vig_im = vig_im.astype(float) / 255.0
    
    res = vig_im / rec_calib_map_rgb
...
def calib_testing(calib_map, rec_calib_map):
    ...
    rmse = np.sqrt(np.mean((rec_calib_map - calib_map)**2))

    abs_error_map = np.abs(rec_calib_map - calib_map)
